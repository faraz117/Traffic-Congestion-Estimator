# -*- coding: utf-8 -*-
"""
Created on Fri Aug 14 22:27:10 2015
...
# shape features = size , aspectRatio , solidity ,width
def checkifonLine(pt,pt1,pt2,window):
...
def main():
    ...
    while True:
        ...
        if roiBox is not None :
                    if(time.time() - timerJSONdump > 15):
                        timerJSONdump=time.time()
                        data = [{'htvcount':count_HGV, 'mtvcount':count_MTV , 'ltvcount':count_LTV}]
                        json.dump(data, open('./data/demo_data.json', 'w'))
                        print "Data Dumped"
     ...
     #filter closing
             ...
             #Box with rotation according to size and angle of contour
                                 ...
                                 if checkifonLine(centroid,(0,0),(maskedHeight-30,maskedWidth-30),4):
                                     ...
                                     #rect_area = w*h
                                     #extent = (float(area)/rect_area)*10
                                     #solidity = (float(area)/hull_area)*10
                                     #equi_diameter = np.sqrt(4*area/np.pi)
                                     objectDetected=roi[y:y+h,x:x+w]
                                     intwidth=int(width)
                                     intheight=int(height)
                                     if area > carSize*15 and area < carSize*250 :
                    ...
                    #cv2.imshow('backgroundSubtract',fgmask)
                        if objectDetected is not None:
                            if prevObject == None:
                                ...
                                #f.write('object: %d,%d,%f,%d,%f,%d,%d'%(count,aspect_ratio,solidity,area,extent,equi_diameter,perimeter))
                                if(carType==1):
                                    #f.write(' HGV\n')
                                    stateHGV=True
                                    timerNow=time.time()                                    
